...
function alertWifi($txt, $hasTimer, $countTimer, $srcImg, $fontSize) {
    ...
    if (!$hasTimer) {
        $btnPanelInfo1 = $("<button></button>").text("Ranking").addClass("button").css("margin-right","20px");
        $btnPanelInfo2 = $("<button></button>").text("Fechar").addClass("button");
        $($contentPanel).append($btnPanelInfo1);
        $($contentPanel).append($btnPanelInfo2);
        $($btnPanelInfo1).click(function(){window.open("ranking.html", "_self");});
        $($btnPanelInfo2).click(function(){closeAlertWifi($panelInfo)});
    }
